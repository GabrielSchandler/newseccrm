...
## 2.0 Correção de entendimento (23/09/2026, confirmado pelo Gabriel)

O modelo alvo **não é** todo usuário podendo pertencer a várias empresas.
É mais simples, e já existe hoje no CRM quase por inteiro:

- **Master** (`is_platform_owner=true`) tem acesso a todas as empresas: cria
  (`/empresas`, `createPlatformCompanyAction`), bloqueia/suspende (campo
  `company_platform_settings.status`, `updateCompanyPlatformSettingsAction`),
  define usuários de qualquer empresa (`/usuarios`, escopado pela empresa
  selecionada) e entra em cada uma (`/empresas/select`, que grava
  `ACTIVE_COMPANY_COOKIE_NAME` e faz `getCurrentUserContext()` resolver
  `companyId` pra empresa selecionada em vez da própria).
- **Gerente, Supervisor e Consultor ficam 1:1 com a própria empresa** — não
  precisam de vínculo N:N nenhum. O modelo atual (`user_profiles.company_id`
  único) já está correto pra esses três papéis; não deve mudar.

Isso reduz drasticamente o que a antiga seção 2.1 abaixo propunha (uma
tabela `company_memberships` N:N para todo mundo). O que falta de verdade
é só **verificar/testar** que o fluxo de master já existente funciona
ponta a ponta (criar empresa → configurar módulos/limite → criar usuário
dentro dela → bloquear → confirmar que bloqueio tira acesso), não construir
um sistema novo de vínculo. O texto abaixo (2.1) fica como registro
histórico de uma proposta maior que não é mais necessária nesses termos.
