...
export function ClientTrackingSection({
  ...
  return (
    <section className="ns-card overflow-hidden">
      <div className="border-b border-[var(--ns-border)] bg-[var(--ns-bg)] px-6 py-5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="ns-card px-4 py-3 text-sm text-[var(--ns-text)]">
      ...
      <div className="grid gap-6 p-6 lg:grid-cols-[minmax(280px,420px)_1fr]">
        <div className="rounded-lg border border-[var(--ns-border)] bg-[var(--ns-bg)] p-4">
          ...
          <div className="mt-4 space-y-4">
            <label className="block">
              <span className="text-xs font-semibold uppercase tracking-wide text-[var(--ns-text-secondary)]">
                Pré-venda / protocolo
              </span>
              <select
            ...
            <label className="flex items-start gap-3 rounded-lg border border-[var(--ns-primary)]/25 bg-[var(--ns-surface)] p-3 text-sm text-[var(--ns-text)]">
              <input
                type="checkbox"
                ...
                className="mt-1 h-4 w-4 rounded border-[var(--ns-border)] text-[var(--ns-primary)] focus:ring-[var(--ns-primary)]"
              />
              <span>
                <span className="block font-semibold text-[var(--ns-text)]">
                  Enviar para aprovação no portal
                </span>
                <span className="mt-1 block text-xs leading-5 text-[var(--ns-text-secondary)]">
            ...
            <button type="button" onClick={handleCreate} disabled={isPending || !preSales.length} className="ns-btn-primary w-full">
        ...
        <div className="min-w-0">
          {orderedUpdates.length ? (
            <div className="relative space-y-4 before:absolute before:left-5 before:top-2 before:h-[calc(100%-1rem)] before:w-px before:bg-[var(--ns-border)]">
                      ...
                      {isEditing ? (
                        <div className="space-y-4">
                          <input
                            ...
                            className="ns-input font-semibold"
                          />
                          <textarea
                          ...
                          <div className="grid gap-3 md:grid-cols-3">
                            ...
                            <input
                              ...
                              className="ns-input"
                            />
                            <label className="flex items-center gap-2 rounded-lg border border-[var(--ns-border)] px-3 py-2 text-sm text-[var(--ns-text)]">
                              <input
                                type="checkbox"
                                ...
                                className="h-4 w-4 rounded border-[var(--ns-border)] text-[var(--ns-primary)] focus:ring-[var(--ns-primary)]"
                              />
                              Solicitar publicação
                            </label>
                          </div>
                          {update.approval_status === "approved" ? (
                            <p className="rounded-lg border border-[var(--ns-warning)]/30 bg-[var(--ns-warning)]/10 px-3 py-2 text-xs leading-5 text-[var(--ns-warning)]">
                              Ao salvar, a aprovação atual será removida e a edição voltará para análise da Gestão.
                            </p>
                          ) : null}
                          <div className="flex flex-wrap gap-2">
                            <button type="button" onClick={() => handleUpdate(update.id)} disabled={isPending} className="ns-btn-primary">
                              Salvar edição
                            </button>
                            <button type="button" onClick={cancelEditing} className="ns-btn-secondary">
                              Cancelar
                            </button>
                          </div>
                        </div>
                        ...
                        <>
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <div>
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                {update.approval_status === "approved" ? (
                                  <span className="inline-flex items-center gap-1 text-xs font-medium text-[var(--ns-success)]">
                                    <Eye className="h-3.5 w-3.5" /> Visível no portal
                                  </span>
                                ) : null}
                              </div>
                              <h3 className="mt-3 text-base font-semibold text-[var(--ns-text)]">
                                {update.title}
                              </h3>
                            </div>
                            <div className="flex gap-2">
                              <button type="button" onClick={() => beginEditing(update)} className="ns-btn-secondary text-xs">
                                <PenLine className="h-3.5 w-3.5" />
                                Editar
                              </button>
                              <button type="button" onClick={() => handleDelete(update)} className="ns-btn-danger text-xs">
                          ...
                          {update.approval_status === "rejected" && update.approval_review_note ? (
                            <div className="mt-3 rounded-lg border border-[var(--ns-danger)]/30 bg-[var(--ns-danger)]/10 px-3 py-2 text-xs leading-5 text-[var(--ns-danger)]">
                              <span className="font-semibold">Ajuste solicitado pela Gestão:</span>{" "}
                              {update.approval_review_note}
                            </div>
                          ) : null}
                          <div className="mt-4 flex flex-wrap gap-3 border-t border-[var(--ns-border)] pt-3 text-xs font-medium text-[var(--ns-text-secondary)]">
            ...
            <div className="rounded-lg border border-dashed border-[var(--ns-border)] bg-[var(--ns-bg)] px-6 py-10 text-center">
              <p className="text-sm font-semibold text-[var(--ns-text)]">
                Nenhuma movimentação registrada ainda.
              </p>
              <p className="mt-2 text-sm leading-6 text-[var(--ns-text-secondary)]">
