...
import { ArrowRight, BookOpenCheck, CheckCircle2, Clock, GraduationCap } from "lucide-react";
...
export default async function AcademyPage({
  ...
  const { supabase, companyId, fullName, userProfileId } = await getCurrentUserContext();
  ...
  return (
    <main className="min-h-screen bg-slate-100">
      <section className="border-b border-slate-200 bg-white px-6 py-8">
        ...
        <div className="mt-2 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
              Treinamento pratico para padronizar atendimento, revisional,
              registro no CRM e protecao dos dados do cliente.
            </p>
          </div>
          <Link
            href={`/academy/capitulos/${firstPending.id}`}
      ...
      <section className="space-y-6 p-6">
        {!progress.tableReady || params.error === "sql" ? (
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
            ...
            <div className="grid gap-3">
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                ...
                <p className="mt-3 text-sm font-medium text-slate-600">Carga estimada</p>
              ...
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                ...
                <p className="mt-3 text-sm font-medium text-slate-600">Capitulos concluidos</p>
                <p className="text-2xl font-semibold text-slate-950">
                  {summary.completedChapters}/{academyCourse.chapters.length}
                </p>
              </div>
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {progress.statuses.map(({ chapter, score, approved, completed }, index) => (
            <Link
              key={chapter.id}
              href={`/academy/capitulos/${chapter.id}`}
